...
//	cout << "eso es" << par[0] << endl;
	alumno[contadoralumnoscreados] = new Alumno{ par[0], par[1], par[3], *grado[1] };//Creamos la clase enviando los parametros aqui tenemos un problema y es que grado no es una string como tal por lo tanto tendremos que añadirlo en un futuro con una clave foranea
//	cout << "es" << par[3] << endl;
		...
		for (int i = 0, flag = -1;i < line.length(); i++)
		{
			...
			if (line.at(i) == ',') {
				...
				if (flag > 2) {
					...
					tipocreate = 6;
					rc = sqlite3_exec(db, sql, callback, NULL, &zErrMsg);
				/*	line.insert(0, "INSERT or IGNORE INTO Disponibilidad (Dia ,Slot, Flag) \nVALUES (");
					line += " ); ";
					cout << line << '\n';
...
int main(int argc, char* argv[]) {
	...
	while (1) {
		printf("\n\nBienvenido al menu De Sqlite de RLX Technologies\n"
			...
			"opcion 12 seleccion de todos profesores\n"
		);
		cin >> opcionmenu1;
