...
generarTribunal(TFG, slot, profesores, tribunal, nprof, doctor){
	...
	for(Profesor profe: profesores){ //por cada profesor
		if(!strcmp(TFG.grado, profe.grado)) continue; //Comparamos si el profesor está en el grado
		
		bool flag = false //Flag para comprobaciones
		for (int hora: profe.disponibilidad.slot){ //Comprobamos si ese slot está disponible para el profesor
			if (slot == hora){
		...
		for(int i = 0; i < nprof; i++){
			if (!strcmp(profe.nombre, tribunal[i].nombre)){ //Comprobamos que no hayamos cogido ya el profesor
